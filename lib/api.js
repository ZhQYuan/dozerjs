...
var api = {
  ...
  process: function (req, res) {
    ...
    if (!endpointMethod.hasOwnProperty('controller') || !endpointMethod.hasOwnProperty('fn')) {
      ...
      modules.lib.stdout('error', 'API error - Not Specified Correctly');
      return;
    }
    ...
    if (!modules.controllers.hasOwnProperty(endpoint)) {
      res.status(500).send('Internal error');
      modules.lib.stdout('error', 'Controller error - Does Not Exist:' + endpoint);
      return;
    }
    ...
    if (!controller.hasOwnProperty(fn) && typeof controller[fn] !== 'function') {
      res.status(500).send('Internal error');
      modules.lib.stdout('error', 'Controller error - Method Incorrect or Does Not Exist: ' + fn);
      return;
    }
